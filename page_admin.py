    ...
    if st.button("Create"):
        ...
        if not role_new:
            ...
            with engine.connect() as conn:
                ...
                check_query = text(
                    "SELECT COUNT(*) FROM user_admin WHERE username = :username"
                )
                if conn.execute(check_query, {"username": user_new}).scalar() > 0:
                    st.error(
                        f'Error: Username "{user_new}" already exists. Please choose a different username.'
                    )
                else:
                    ...
                    conn.execute(
                        insert_query,
                        {"username": user_new, "password": pass_new, "role": role_new},
                    )
                    conn.commit()
...
def delete_user():
    ...
    with engine.connect() as conn:
        query = text(
            "SELECT user_id, username, role FROM user_admin WHERE role != 'admin'"
        )
    ...
    if result:
        ...
        if st.button("Delete User"):
            try:
                with engine.connect() as conn:
                    delete_query = text(
                        "DELETE FROM user_admin WHERE username = :username"
                    )
...
def change_password():
    ...
    if st.button("Confirm"):
        ...
        if not pass_lama:
            ...
            with engine.connect() as conn:
                query = text(
                    "SELECT password FROM user_admin WHERE username = :username"
                )
                result = conn.execute(
                ...
                if result and pass_lama == result[0]:
                    ...
                    conn.execute(
                        update_query,
                        {"username": st.session_state.username, "password": pass_baru},
                    )
                    conn.commit()
...
def main():
    ...
    if st.session_state.role == "admin":
        menu_option = st.sidebar.selectbox(
            "Admin Menu", ("Create User", "Delete User", "Change Password")
        )
